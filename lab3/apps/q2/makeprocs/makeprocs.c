...
*/
  ...
  for (num =0; num < num_h2o; num++){
    process_create("one.dlx.obj", 0, 1, s_procs_completed_str, h2o_mbox_str);
  }

  for (num =0; num < num_so4; num++){
    process_create("two.dlx.obj", 0,1, s_procs_completed_str, so4_mbox_str);
  }

  for (num =0; num < num_three; num++){
    process_create("three.dlx.obj", 0,1, s_procs_completed_str, h2o_mbox_str, h2_mbox_str, o2_mbox_str);
  }

  for (num =0; num < num_four; num++){
    process_create("four.dlx.obj", 0,1, s_procs_completed_str, so4_mbox_str, so2_mbox_str, o2_mbox_str);
  }
  
  for (num =0; num < num_five; num++){
    process_create("five.dlx.obj", 0,1, s_procs_completed_str, h2_mbox_str, o2_mbox_str, so2_mbox_str, h2so4_mbox_str);
  }
