...
cond_t CondCreate(lock_t lock) {
  ...
  // checking if the lock is valid
  if (lock < 0 || lock >= MAX_LOCKS) {
	//dbprintf("Lock %d passed to CondCreate is not a valid lock!\n", lock);
    return INVALID_COND;
  }
  if (locks[lock].inuse == 0) {
	dbprintf('s', "Lock %d passed to CondCreate is not a valid lock in use!\n", lock);
    return INVALID_COND;
  }
  ...
  RestoreIntrs(intrval);
  // if condition variable cannot be created (no more condition variables left)
...
int CondInit(Cond *c) {
  ...
  //c->pid = -1;  // not sure if I need this
  return SYNC_SUCCESS;
}
...
int CondWait(Cond* k) {
  ...
  // Check to see if the calling process owns the lock
  if (locks[k->lock].pid != GetCurrentPid()) { // if calling process does NOT own the lock
	dbprintf('s', "CondWait: Proc %d does NOT own the lock %d\n", GetCurrentPid(), k->lock);
	//dbprintf('s', "CondWait: Proc %d does NOT own the lock %d, which is associated with this condition variable %d\n", GetCurrentPid(), k->lock, int(k-conds));
   //dbprintf('s', "SemWait: Proc %d granted permission to continue by sem %d\n", GetCurrentPid(), (int)(sem-sems));
	RestoreIntrs(intrval);
	return 1; //SYNC_FAIL; //or 1? check with TA
  }
  if(LockHandleRelease(k->lock) == SYNC_FAIL)  {  // releasing the lock
	dbprintf('s', "CondWait: Bad lock_release for lock%d.\n", k->lock);
    exitsim();
  }
  // make the process go to sleep
  dbprintf('s', "CondWait: putting process %d to sleep\n", GetCurrentPid());
  if ((l = AQueueAllocLink ((void *)currentPCB)) == NULL) {  // making a link for the current process
	printf("FATAL ERROR: could not allocate link for lock queue in CondWait!\n");
	exitsim();
  }
  if (AQueueInsertLast (&k->waiting, l) != QUEUE_SUCCESS) { // adding link to queue of waiting processes
	printf("FATAL ERROR: could not insert new link into cond waiting queue in CondWait!\n");
	exitsim();
  }
  ProcessSleep();
  RestoreIntrs(intrval);
  return 0; //SYNC_SUCCESS;  // or 0? double-check appropriate return value with TA
}
...
int CondSignal(Cond* k) {
  ...
  intrs = DisableIntrs ();
  dbprintf ('s', "CondSignal: Proc %d signalling lock %d using condition variable %d.\n", GetCurrentPid(), k->lock, (int)(k-conds));

  if (locks[k->lock].pid != GetCurrentPid()) {  // if calling process hasn't acquired the lock
	dbprintf('s', "CondSignal: Proc %d does not own lock %d.\n", GetCurrentPid(), k->lock);
	return 1; // SYNC_FAIL;  // return 1;
  }
  //[k->lock].pid = -1;
  if (!AQueueEmpty(&k->waiting)) { // there is a process to wake up
	l = AQueueFirst(&k->waiting); 
	pcb = (PCB *)AQueueObject(l);
	if (AQueueRemove(&l) != QUEUE_SUCCESS) { // pop that link off the queue
	  printf("FATAL ERROR: could not remove link from lock queue in CondSignal!\n");
	  exitsim();
	}
	dbprintf ('s', "LockRelease: Waking up PID %d.\n", (int)(GetPidFromAddress(pcb)));
	//[k->lock].pid = GetPidFromAddress(pcb);
	ProcessWakeup (pcb);
  }
  RestoreIntrs (intrs);
  return 0; //SYNC_SUCCESS;  // return 0;
}
...
int CondBroadcast(Cond* k) {
  ...
  intrs = DisableIntrs ();
  dbprintf ('s', "CondBroadcast: Proc %d signalling lock %d using condition variable %d.\n", GetCurrentPid(), k->lock, (int)(k-conds));
