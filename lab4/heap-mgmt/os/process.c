...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    for (j = 0; j < PROCESS_HEAPSIZE; j++){
      pcbs[i].h_blocks[j].inuse = 0;
    }
    pcbs[i].blockCounter = 0;
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  uint32 heapPage; //tmp page for heap initialization
  //int heap_ind;
  Link *h_link;
  ...
  //h_ind = 0;
  pcb->h_blocks[pcb->blockCounter].vaddr = (void *)(4 * MEM_PAGESIZE); //starting address of block after
  pcb->h_blocks[pcb->blockCounter].inuse = 0;
  pcb->h_blocks[pcb->blockCounter].size = MEM_PAGESIZE;

  AQueueInit(&(pcb->h_queue));
  h_link = AQueueAllocLink(&(pcb->h_blocks[pcb->blockCounter]));
  AQueueInsertFirst(&(pcb->h_queue), h_link);
